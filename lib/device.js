...
module.exports = Thing.type(Parent => class extends Parent.with(Polling) {
	...
	constructor(handle) {
		...
		this._propertiesToMonitorWithCommand = [];
	...
	defineProperty(name, def) {
		if (typeof name.cmd !== 'undefined') {
			this._propertiesToMonitorWithCommand.push(name);
			name = name.name;
		} else {
	...
	async poll(isInitial) {
		for(let i =0; i < this._propertiesToMonitorWithCommand.length; i++) {
			const cmd = this._propertiesToMonitorWithCommand[i].cmd;
			const prop = [this._propertiesToMonitorWithCommand[i].name];

			await this._loadProperties(prop, cmd);
		}
		// Polling involves simply calling load properties
		return this._loadProperties();
	}

	_loadProperties(properties, cmd) {
		...
		return this.loadProperties(properties, cmd)
			.then(values => {
